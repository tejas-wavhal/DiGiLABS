      ...
      <section className="mx-5 flex justify-center my-7 md:mx-auto md:mt-52 lg:mt-0 lg:flex lg:justify-evenly lg:items-center lg:space-x-0 md:mr-6 relative lg:bottom-10">
        <form className="mt-1 max-w-[344px] md:mx-auto lg:mx-0 " onSubmit={handleOnSignup}>
          <div className="-space-y-3 mb-8">
            <h1 className="text-[28px] font-bold">Welcome to</h1>
            <h2 className="text-[28px] font-bold">Systempackage</h2>
          </div>
          <input type="text" value={username} onChange={(e) => setUsername(e.target.value)} className="bg-[#f3f3f3] text-gray-900 rounded-xl block w-full p-4 h-[56px] font-semibold border-none" placeholder="Username" />
          ...
          <div className='flex float-right relative bottom-16 right-5'>
            {showPassword === 'password' ? (<button onClick={() => setShowPassword('text')}><svg width="16" height="24" viewBox="0 0 16 24" fill="none" xmlns="http://www.w3.org/2000/svg" className='mr-[0.10rem]'>
              ...
              <button onClick={() => setShowPassword('password')}><BiShow className='text-[#7a7a85] text-xl mt-[0.10rem] mb-[0.15rem] ' /></button>)}
          </div>
          <button className="font-semibold bg-maincolor-100 hover:bg-maincolor-200 rounded-xl p-2 text-white w-full h-[56px] relative bottom-3" type='submit'>Next</button>
          <Link to='/verify'><p className="text-maincolor-100 font-bold my-2 text-center cursor-pointer hover:underline">Forgot your password?</p></Link>
        </form>
        <Slider />
      </section>
